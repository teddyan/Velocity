...
class AdminCCLExamManagement extends Component{
    state={
        loading:true,
        CCLExamInfo:[],
    ...
    componentDidMount() {
        ...
        axios.get(global.config.url + 'AdminHappy/PaperInfo?type=ccl',{headers: {Authorization: `Bearer ${token}`}}).then(res=>{
            console.log(res.data)
            // check Msg to double verify
            if(res.data.msg=='succeed'){
                // use loop to load the response data to the temporarily variable
                res.data.data.map(data=>{
                    tempcclExamInfo.push(data)
                })
                // set state
                this.setState({loading:false, CCLExamInfo:tempcclExamInfo})
                console.log(this.state.CCLExamInfo)
            }else{
        ...
        return(
            <React.Fragment>
                ...
                >
                    ...
                    <TableHeaderColumn dataSort={ true } dataFormat={this.CCLFormatting} hiddenOnInsert editable={ false } dataSort={false}><span style={HeaderSpanWord}>CCL</span></TableHeaderColumn>
    ...
    CCLFormatting(cell, row){
        return(
            <React.Fragment>
                <a href={'/AdminManagement/AdminCCLExamManagement/CCLInsertAll?Paper='+row.Paper_ID}>CCL</a>
            </React.Fragment>
        )
    }
    ...
    onAddRow(row) {
        ...
        axios.post("http://localhost:8000/" + 'AdminHappy/InsertCCLPaper', {isFree:row.isFree, papername:row.Paper_Name},{headers: {Authorization: `Bearer ${token}`}}).then(res=>{
            if(res.data.msg=='succeed'){
...
export default AdminCCLExamManagement;