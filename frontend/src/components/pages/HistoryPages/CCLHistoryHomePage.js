...
class CCLHistoryCCLHomePage extends Component {
                ...
                if (data.Status === -1) {
                    ...
                    if ((data.Status === 1 && data.isExpert === 1) || (data.Status === 1 && data.isExpert === 0 && data.isFree === 0)) {
                        ...
                        TemExamHistory.push([
                            ...
                            data.Paper_Name,
                            data.CreateAt,
                            data.Dialogue1_Score,
                            data.Dialogue2_Score,
                            data.FinishScore,
                            '已出分',
                            data.isAudioExist,
                            commentState
    ...
    render() {
        return (
            <React.Fragment>
                ...
                <div style={{marginTop: '13px', color: global.config.red}}>点击查看点评(口语)答案</div>
                <MDBTable hover className='finalRound'
                    ...
                    <MDBTableHead style={{backgroundColor: global.config.brown}} textWhite>
                        <tr style={{cursor: 'pointer'}}>
                            ...
                            <th onClick={() => this.sortColumn(3)} style={filerTextStyle}>Section1 {this.sortState[3] === 0 ? doubleArr : this.sortState[3] === 1 ? upArr : downArr}</th>
                            <th onClick={() => this.sortColumn(4)} style={filerTextStyle}>Section2 {this.sortState[4] === 0 ? doubleArr : this.sortState[3] === 1 ? upArr : downArr}</th>
                            <th onClick={() => this.sortColumn(5)} style={filerTextStyle}>总分 {this.sortState[5] === 0 ? doubleArr : this.sortState[4] === 1 ? upArr : downArr}</th>
                            <th onClick={() => this.sortColumn(6)} style={filerTextStyle}>分数状态 {this.sortState[6] === 0 ? doubleArr : this.sortState[5] === 1 ? upArr : downArr}</th>
                            <th onClick={() => this.sortColumn(7)} style={filerTextStyle}>点评状态 {this.sortState[7] === 0 ? doubleArr : this.sortState[6] === 1 ? upArr : downArr}</th>
                            <th style={{
                                fontSize: '14pt',
                                borderRadius: '0 20px 0 0',
                                verticalAlign: 'middle'
                            }} onClick={() => this.sortColumn(8)}>录音下载 {this.sortState[6] === 0 ? doubleArr : this.sortState[8] === 1 ? upArr : downArr}
                            </th>
                        </tr>
                    </MDBTableHead>
                                ...
                                return (
                                    <tr key={record[0]}>
                                        ...
                                        <td style={size12} className='SS'>
                                            <a href={'/CCLHistoryPage/CCLSpeakingPage?' + record[0]}>{record[3]}</a>
                                        </td>

                                        <td style={size12} className='SS'>
                                            <a href={'/CCLHistoryPage/CCLSpeakingPage?' + record[0]}>{record[4]}</a>
                                        </td>
                                        <td style={size12}>{record[5]}</td>
                                        <td style={size12}>{record[6]}</td>
                                        <td style={size12}>{record[7] === 0 ?
                                            <Button onClick={() => {
                                                this.setState({showModal: true, examID: record[0]});
                                            }}>申请点评</Button> : (record[8] === 1 ? '待点评' : (record[8] === 2 ? '已点评' : '已放弃'))}</td>
                                        <td style={size12}>{record[7] === 1 ?
...
export default CCLHistoryCCLHomePage;